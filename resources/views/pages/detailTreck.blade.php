        ...
        <div class="d-flex flex-row mb-3">
            <div class="d-flex flex-column">
                <div class="pull-right">
                    <a class="btn btn-primary mt-3" href="{{ route('home') }}" enctype="multipart/form-data">
                        Back</a>
                </div>
                <h2>Nom : {{ $treck->treckName }}</h2>
                        ...
                        <div class="d-flex flex-row mb-3">
                            <div class="d-flex flex-column">
                                <div class="">
                                    ...
                                    <span class="ms-5 fw-bolder">Time : {{ $treck->time }}min</span>
                                    ...
                                    <div class="d-flex flex-column">
                                        @if (Auth::user()->admin == false && Auth::user()->id == $treck->idUser && $treck->private == true)
                                            <form action="{{ route('treck.destroy', $treck->id) }}" method="Post">
                                                ...
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger"
                                                    onclick="return confirm('Are you sur to delete {{ $treck->treckName }} ?');">Delete
                                                    Treck</button>
                                            </form>
                                        @elseif (Auth::user()->admin == true && $treck->idUser == Auth::user()->id)
                                            <form action="{{ route('treck.destroy', $treck->id) }}" method="Post">
                                            ...
                                            <a class="btn btn-info mt-3" href="{{ route('treck.edit', $treck->id) }}">Modify
                                                Treck</a>
                                        @endif
                                    </div>
                                @endauth
